...
CLASS ltcl_integration_test IMPLEMENTATION.
  ...
  METHOD create_travel.
    ...
    DATA travels_in TYPE TABLE FOR CREATE zi_rap_travel_U_987\\travel.
    ...
    MODIFY ENTITIES OF zi_rap_travel_u_987
        ENTITY travel
    ...
    SELECT * FROM zi_rap_travel_u_987 WHERE TravelId = @new_travel_id INTO TABLE @DATA(lt_travel)  .
  ...
  METHOD class_setup.
    cds_test_environment = cl_cds_test_environment=>create_for_multiple_cds(
        i_for_entities = VALUE #( ( i_for_entity = 'zi_rap_travel_u_987' )
                                  ( i_for_entity = 'zi_rap_booking_u_987' ) )
                                ).
  ENDMETHOD.
